...
load("/Users/weath/Documents/citations-2024/philo_bib.RData")
load("/Users/weath/Documents/citations-2024/philo_cite.RData")
...
most_cited_articles <- article_times_cited |>
  ...
  select(Article = longcite, Citations = citations)


most_citing_articles <- article_times_citing |>
  slice_max(order_by = citations, n = 10) |>
  left_join(select(philo_bib, id, longcite), by = c("new" = "id")) |>
  select(Article = longcite, `Articles Cited` = citations)
