...
articles_per_year <- active_philo_bib |>
  ...
  group_by(old_year) |>
  tally(name = "articles") |>
  mutate(available = slide_dbl(articles, sum, .before = 14) - articles)
...
# Denominator is (all_cites(new_year) - cites(old_year, new_year)) / (avail_articles - pubs(old_year))
...
age_effect_tibble <- year_in_year_out |>
  ...
  mutate(available = case_when(
    age >= 1 & age <= 14 ~ available - articles,
    TRUE ~ available
  )) |>
  filter(available > 0) |>
  mutate(cite_ratio = 
...
Although the number of citations is going up, the number of articles available to be cited is also going up. For reasons I'll get to in @sec-age, I'll say articles are _available_ to be cited if they were published between 0 and 20 years before the article doing the citing. Now of course some articles are cited more than 20 years after publication, and some articles are cited before they are published. But the large majority of citations take place in this window, and it will help focus our analysis to restrict attention to them. Using that definition, the number of available articles each year is shown in @fig-availablearticles.
...
Putting all these together we can work out how often, on average, available articles are cited in each year. The results are in @fig-availablerate. (I haven't graphed this, but the numerator in @fig-availablerate is the number of citations to articles between 0 and 20 years old. That's similar to, but not quite identical to, the number of citations altogether.)
