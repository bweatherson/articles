...
This paper is a discussion of some of the philosophically interesting questions in non-ideal decision theory. It has three main aims. The first is to argue that philosophical work on decision theory has been too focused on the idea case, where the decision maker is logically and mathematically omniscient, and faces no computational costs. This is an important special case, but it is a special case, and we should also pay attention to the more general case where computational costs are non-zero. The second is to argue that the formal structure of a non-ideal decision theory should be very different to the formal structure of ideal decision theory. Ideal decision theory makes a binary division of choices into the rational and irrational. Non-ideal decision theory makes a more fine-grained division of choices into the more and less skilled. And the third, and most contentious, aim is to argue against a broadly evidentialist approach to non-ideal decision theory, and in favor of a broadly reliabilist approach. I'll argue that skilled choices are those that result from processes that reliably produce good results. The most salient contrast will be with views that identify skilled choice with those that result from processes that the chooser has reason to believe, or has evidence, reliably produce good results. I'll be arguing that the fact of reliable success is more important than evidence or belief about it. This view is vulnerable to an obvious objection: it counts as skilful choices that result from a lucky guess about which approach to take, and this seems intuitively wrong. I'll argue that when we look at the details of individual cases, this objection looks less pressing.

<!--Check number of cities, and that map works-->

# Decisions Under Certainty {#deccertainty}

Most philosophical work on decision theory focusses on choices under uncertainty. But it will be useful here to start with an example of decision making under certainty. In particular, I'll start with a version of the problem that Julia Robinson dubbed the 'travelling salesman' problem.~^[The dubbing is in @Robinson1949. For a thorough history of the problem, see @Schrijver2005. For an accessible history of the problem, which includes these references, see the wikipedia page on 'Traveling Salesman Problem'.]~ 

>> **Salesman**    
>> Chooser is given the straight line distance between each pair of cities from the twenty-one represented on the map below. Using this information, Chooser has to find as short a path as possible that goes through all twenty-one cities and returns to the first one. The longer a path Chooser selects, the worse things will be for Chooser. 
...
Since there are $20!$ possible paths, Chooser has a few options here.^[The twenty-one cities are the cities in California, Nevada and Oregon included in the 312 cities in North America that John Burkardt mapped in his dataset Cities, available at <people.sc.fsu.edu/~jburkardt/datasets/cities/cities.html>. Those cities are: `r str_c(our_gps$name, sep = "; ", collapse = "; ")`.]

Question: What makes a particular choice of path the best one? Answer: It's no longer than the other ~$20!-1$~ options.

Question: What is the best way for Chooser to make a choice here? Answer: Read a lot of the literature on this kind of optimisation problem, look for ways to rule out large collections of options all at once, etc.

These answers are quite different, and that means the questions are different too. The first question is asking something almost metaphysical; in virtue of what is the right answer the right one? The second question is practical; how do we go about finding that right answer, or at least getting close to it?

It would be really terrible to answer the second question by saying "choose the shortest route". This wouldn't be obviously incorrect. But it would be useless, which in context is almost as bad. The second question is asking a question in what we might call non-ideal decision theory.^[I'm borrowing the term 'non-ideal' from work in political philosophy. See @Valentini2012 for a good survey of some relevant work, and @Mills2005 for an important critique of the centrality of ideal theory in political philosophy. Critics of ideal theory, such as Mills, and @Sen2006, argue that we shouldn't base non-ideal theory on ideal theory. I'm going to agree, but my focus is primarily in the other direction. I'm going to argue that it isn't a constraint on ideal theory that it is useful in constructing a non-ideal theory.] And a constraint on a non-ideal theory is that it is useful, in a way that "choose the shortest route" is useless. But this is the right answer to the first question. The shortest route is, kind of by definition in this case, the best choice. That's the ideal. An ideal decision theory should say to choose it.


<!---Intro about basketball example from Demon book
     Have to adjust table; it won't print right like this
     Also have to introduce language about ideal, maybe earlier--->

# Decisions Under Uncertainty {#decuncertain}

Now there aren't many decision theory papers in philosophy journals that focus on problems, like Salesman, where all the relevant facts are available to Chooser. So let's regain contact with the literature by looking at a problem that does involve uncertainty.
...
Any philosophical decision theory I know of says that in Basketball, Chooser should Pass. This point at least is common ground in the literature.^[There is one possible exception. Lara @BuchakRisk develops a risk-sensitive theory of decision. On Buchak's theory, if Chooser is risk-attracted, it could be rational for them to bet. But while this is a possible use of Buchak's theory, it's not really the intended use. Her focus, and the focus of most of the discussion of her work, is on the use of the theory to model risk-averse choosers. And any Buchakian risk-averse chooser will also Pass in Basketball. This example is structurally like the much-discussed miners' puzzle, but with a somewhat more realistic, and less grizzly, story.] That's not the optimal outcome for Chooser. The optimal outcome is that they bet on the winning team. But since they don't know who that is, and either bet will, on average, lose them money, they should Pass rather than bet on Home or Away.
...
- Better way to put this. Reliabilism gets the cases right, and seems to have the right kind of structure. It needs a theory of action, a la Mandlekern. And it needs a theory of reference in Newcomb cases. But that's ok - could also explain the ambivalence, since there are two natural reference classes (holding fixed content, holding fixed evidence).

- Big problem is that it seems to make too much room for luck. Imagine that Fargle and Nargle don't know that furthest insertion is typically better than nearest insertion. Is Fargle more rational for using it? Intuitively no, but three responses.

1. Should think of a theory of non-ideal decision as measuring skill, not rationality. It's not just measuring results; sometimes a less skilled performer will do better than a more skilled one due to luck. But it's not rationality either. Skill is clearly dependent on external features. How skilled a hitter is in baseball is a function of what kind of pitches typically get thrown.
2. Should look at the details about why Fargle and Nargle made their selections. Assume that Fargle had a good feeling about furthest insertion. Then one strategy would be to say the algorithm followed isn't furthest insertion, it's doing what Fargle has a good feeling about. Is that typically successful? If so, not sure it isn't skilled. If not, then perhaps it isn't really reliable.
3. Political cases, a la Srinivasan, have different intuitions here. A member of a discriminated against group who instincively avoids situations where they will be sharply discriminated against, even if they don't think that's what they are doing, is arguably showing the kind of skill non-ideal decision theory measures. We shouldn't think that the skills involved in skilfully navigating oppressive social structures will always be visible to the person exercising them; that's part of what it means to be oppressed. But they are still skills.
