...
Perhaps the most intuitive case for indecisiveness involves what I'll call Stag Hunt decisions.[^8] Here is an example of a Stag Hunt decision.

[^8]: For much more on the philosophical importance of Stag Hunts, see @Skyrms2004.
...
That's incredibly abstract, but it excludes some possibilities. It excludes cases where Chooser learns along the way that they have hitherto unknown abilities. It excludes cases where Chooser gains the capacity to think new relevant thoughts along the way, say by meeting a new person and gaining the capacity to have singular thoughts about them.[^10] But it does cover a lot of cases.

[^10]: Following @Stalnaker2008, I think it excludes the Sleeping Beauty case, since there Beauty gains the capacity to have singular thoughts about a time, the 'now' when she awakes, that she did not previously have.
...
Start with why it is bad to just have a resolute approach.[^12] Game theorists usually reject this approach because it means sometimes making a decision that one knows will have worse consequences than an available alternative. I'll go over an example of this, though I should note it is rather violent. This is unavoidable; it is only in these violent cases that we can be sure the Chooser is really making things worse, and not acting for a strategic or reputational goal.

[^12]: The so-called Foundational Decision Theory of @LevinsteinSoares2020 agrees with the resolute approach in the special case where the only information Chooser will receive are the results of predictions, and is subject to the criticisms I'll make of resolute theories.
...
What about the other direction? Is it sensible to have a sophisticated theory that is not resolute? There does seem to be something puzzling about such theories. They are "diachronically exploitable" in the sense described by @Spencer2021. Here is Spencer's example of a diachronically exploitable agent.[^13] The agent will play first the left and then the right game, and their payouts (shown in dollars) will be summed over the game. They won't be told between the games what they got from the first game.[^14]

[^13]: Spencer himself thinks diachronic non-exploitability is a constraint on rationality, but rational agents do not violate a similar constraint he calls the Guaranteed Principle. I don't find it plausible that one but not the other is a constraint on rationality.

[^14]: Assume Chooser is reasonably risk-neutral over dollars over this range of outcomes.
...
Newcomb Insurance (from @Spencher2021).
:::
...
: Strategic form of Newcomb Insurance. {#tbl-ni-strategic}
...
: First game in Newcomb Insurance, if D~2~ will be played. {#tbl-ni-new-left}

And while that game has multiple solutions, it is clear that it is permissible to play U~1~. Moreover, it is also rational to play a 50/50 mixture of the two options in @tbl-ni-left, and then D~2~ in @tbl-ni-right, for an expected return of \$25.

So this example isn't a reason to believe in the rationality of diachronic exploitability. But is there a positive argument we can give against diachronic exploitability? There are some circumstances where one ought adopt a sub-optimal strategy. Odysseus binds himself to the mast because he does not approve of future-Odysseus's preferences. Professor Procrastinate[^15] cite turns down a referee request because he does not trust his future self to be practically rational. Both of them deliberately turn down strategies that would be better than where they end up, because they do not trust their future selves to carry them out. They are alienated in this way from their future selves.

[^15]: A famous character in @JacksonPargetter1986.
...
3.  Demon, knowing what Chooser has said, and being arbitrarily good at predicting Chooser's strategy[^17]. will say Heads if it is more probable the coin landed Heads, and Tails if it is more probable the coin landed Tails.[^18]
...
[^17]: That is, what Chooser will do if Heads, and what they will do if Tails.

[^18]: If both are equally likely, Demon will flip a fair coin and say how it lands.
