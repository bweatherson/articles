...
gameformat <- function(game, caption){
  ...
  kbl(game,
    ...
    sub("\\\\toprule", "", .) %>%
    sub("\\\\bottomrule", "", .)
  #### Huxtable version for HTML
  # as_hux(game) %>%
  #   # set_markdown_contents(1, 1, "") %>%
  #   set_markdown(everywhere, everywhere) %>%  
  ...
  #### Flextable version for Word
  #### Note have to include " " for first column, not ""
  #### Remember to include References line iff in Word (gotta be a way to automate this)
  # flextable(game) %>%
  #   colformat_md() %>%
  #   border_remove() %>%
  #   hline(part = "header") %>%
