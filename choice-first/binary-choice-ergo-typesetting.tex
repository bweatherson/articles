...
similar examples to argue that preferences form a semiorder.
...
\emph{xRy}~∨~\emph{yRx}~∨~x=y.) Further, let \emph{C}(\emph{S}) be the
set of Pareto optimal members of \emph{S} relative to those orderings.
That is, it is the set of members of \emph{S} such that no other member
of \emph{S} is better according to every member of the set of orderings.
...
preference is to explain trade, and it is natural to give preferences a
...
and Down. Whatever probability Row assigns to Left/Right, Middle will
...
one other option, but not when there are two. So there's no binary
