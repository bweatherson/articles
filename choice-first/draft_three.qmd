...
A central example which he uses, and which @Sen2004 picks up on, involves an executive looking to hire a secretary. I'll follow Sen and call the executive Ms. Jones. She is looking for a secretary with good typing skills and good stenography skills. (This is the 1980s.) We'll conceive of these, a bit arbitrarily, as distinct values. There are three candidates: Jane, Dolly, and Lilly, and their value on each measure is given in @tbl-secretaries. (I've slightly adjusted the numbers to match the earlier examples.)
...
Levi argues that if the numbers are like this, and Dolly is barely better than the other two on each metric, she is not choiceworthy. That's true even though she might be choiceworthy if one or the other candidates was unavailable.
...
This is enough to rule out the example Sen has in mind on the first horn, where a secretary with skill 1 on either dimension is more than half as valuable as a secretary with skill 10. But it's not enough to say that Ms. Jones should not hire Dolly. Unidimensional Expectations is consistent with one resolution of her indeterminate value being that the value of \langle*x*,*y*\rangle is *xy*. If that's a permissible resolution, then there will be a resolution on which Dolly is maximally valuable. But there are further principles that do rule out Dolly. The most intuitive argument I know uses the following.
