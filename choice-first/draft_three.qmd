...
format:
  ...
  pdf: 
    ...
    linestretch: 1.75
    link-citations: true
    output-file: "Are Choices Binary"
    include-in-header:
      text: |
...
The usual way these cases are discussed, starting with Luce and Raiffa, is that they violate a certain kind of comparability. For example, Luce and Raiffa say there is a sense in which the two holidays are 'not comparable'. I want to resist this reading. The core intuition here, I think, is that \beta fails. Chooser would choose either option from {*a*, *b*}, but if *c* is added as an option, *a* becomes unchoiceworthy. If we add the assumption that *R* = *B*(*C*), then it does follow that trichotomy fails, and there is a sense in which they are incomparable. But without that assumption, it's consistent to say that these are counterexamples to \beta but not to trichotomy. We'll return to this point in @sec-dorr.

# Properties \gamma and \delta {#sec-gamma}

Assume *R* does not satisfy trichotomy, but is a semiorder, and *C* = *B*(*R*). Then \beta will fail, but \gamma and \delta will hold. Conversely, for any *C* where \gamma and \delta hold, there is a semiorder *R* such that *C* = *B*(*R*) [@Sen1970sec 320]. We're not going to be very interested in \delta, but we will be very interested in \gamma. 
...
@LuceRaiffa1957 discuss what they call choices under 'uncertainty', by which they mean choices where Chooser cannot assign probabilities to the states. @Petersen2017 calls these choices under 'ignorance'. None of the proposed decisive rules for choice under uncertainty/ignorance are particularly compelling; all lead to very strange outcomes. 
...
If that's right, then the three examples from @sec-games can be repurposed as examples of choice under uncertainty, replacing Left and Right with *p* and \neg*p*, and the same analysis will hold. Again \gamma will fail, because Middle is choiceworthy when there is one other option, but not when they are two. So there's no binary comparison of Middle with the other two options that explains the facts about what is choiceworthy in the three cases.
...
Jack @Spencer2023 argues against views, like that defended by Dmitri @Gallow2020, which say only Up is choiceworthy in @tbl-nice-demon. His argument relies on a simple principle. If Chooser plans to play Down, then Chooser knows Down will have the best return, and it's not irrational to make the choice one knows will have the best return. @Gallow2024 argues that knowledge might not be a strong enough state to make this principle work, because of high stakes gambles. I think knowledge should play the role in practical reasoning Spencer assigns it (@WeathersonKAHI), so I'll assume Spencer is right here, and both Up and Down are choice-worthy in @tbl-nice-demon. Indeed generally in any problem with a nice demon, any option that would be best were it chosen, is choiceworthy for just the reason Spencer gives.
...
In @tbl-nice-demon-exit, Exit is not choiceworthy. Whatever credences Chooser has about what Demon has done, it is better in expectation to choose one of Up or Exit. But if either Up or Down were unavailable, Exit would be choiceworthy. So again this case is a counterexample to \gamma.
...
Most of the arguments in this paper against the binaryness of choice turn on counterexamples to \gamma, but this is a distinct argument. Sometimes, as in Rock-Paper-Scissors, there are grounds for rational choice, but no grounds for rational preference. The only preferences that would ground the choice would violate (@notm). So rational choiceworthiness cannot be grounded in rational preference.
...
But the case might still be a counterexample to \gamma. Consider these two variants on the case.
...
:    L\langle*x*~1~, *y*\rangle\langle*x*~2~, *y*\rangle = \langle(*x*~1~ + x~2~)/2, *y*\rangle
:    L\langle*x*, *y*~1~\rangle\langle*x*, *y*~2~\rangle = \langle*x*, (*y*~1~ + y~2~)/2\rangle
...
This is enough to rule out the example Sen has in mind on the first horn, where a secretary with skill 1 on either dimension is more than half as valuable as a secretary with skill 10. But it's not enough to say that Ms. Jones should not hire Danny. Unidimensional Expectations is consistent with one resolution of his indeterminate value being that the value of \langle*x*,*y*\rangle is *xy*. If that's a permissible resolution, then there will be a resolution on which Danny is maximally valuable. But there are further principles that do rule out Danny. The most intuitive argument I know uses the following.

Substitution of Identicals (SI) 
:    If *x* = *y*, then *Lxz* = *Lyz* and *Lzx* = *Lzy*.

No Trade-Off (NT)
:    L\langle*x*, *x*\rangle\langle*y*, *y*\rangle = \langle(*x* + *y*)/2,(*x* + *y*)/2
...
|    |   |   |
|--:|:--|:--|
| 1. | \langle5, 5\rangle = *L*\langle10, 5\rangle\langle0,5\rangle | UD |
| 2. | \langle10, 5\rangle = *L*\langle10, 10\rangle\langle10,0\rangle | UD |
| 3. | \langle0, 5\rangle = *L*\langle0, 10\rangle\langle0,0\rangle | UD |
| 4. | \langle5, 5\rangle = *L*(*L*\langle10, 10\rangle\langle10,0\rangle)\langle0,5\rangle | 1,3 SI |
| 5. | \langle5, 5\rangle = *L*(*L*\langle10, 10\rangle\langle10,0\rangle)(*L*\langle0, 10\rangle\langle0,0\rangle) | 2, 4 SI |
| 6. | \langle5, 5\rangle = *L*(*L*\langle10, 10\rangle\langle0,0\rangle)(*L*\langle0, 10\rangle\langle10,0\rangle) | 5 RO |
| 7. | \langle5, 5\rangle = *L*\langle10, 10\rangle\langle0,0\rangle | NT |
| 8. | \langle5, 5\rangle = *L*\langle5, 5\rangle(*L*\langle0, 10\rangle\langle10,0\rangle) | 6, 7 SI |
| 9. | \langle5, 5\rangle = *L*\langle0, 10\rangle\langle10,0\rangle | 8 WI |
...
Given \alpha and \gamma, this will entail Lederman's version of the principle. But without them, it does not.

Another argument Lederman gives more explicitly assumes \gamma. He says (with several changes made to match the examples):
...
That would be bizarre, but violating Negative Dominance does not imply that it is rationally permitted. To get that it is, you need \gamma, since you need that if (a) is choiceworthy from {(a), (b)} and from {(a), (c)}, it is choiceworthy from {(a), (b), (c)}.

But the biggest argument Lederman gives is that we need to explain why Ms. Jones prefers the Jack/Luke lottery to Danny, and that to offer the fact that the lottery does better _in expectation_ on both values is to "fetishize global features of games" [@Lederman2025 22]. Now I think the explanation for why the lottery is better is that Danny \notin *C*({Jack, Danny, Luke}), and I'm not sure why that's a "global" feature of a game while facts that Lederman takes to be reasonable grounds, such as which secretaries are in *C*(*S*) for various pair sets, is not. But set that aside; there is a cleaner explanation to offer, as follows:
...
3. Any choice function that violates \gamma cannot be generated from a preference relation, and there are many reasons for endorsing choice functions which violate \gamma.
