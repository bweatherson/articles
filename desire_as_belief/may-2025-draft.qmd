...
Peter is currently contemplating some action *a*. Let A be the proposition that Peter does *a*, and Å be the proposition that doing *a* is good. Unless stated otherwise, I'll follow Lewis in making the simplifying assumption that all actions are good or bad. This simplifies a lot, though eventually we'll have to drop it because it simplifies too much. In this case, I'll also make the similar simplifying assumption that it's common knowledge that not doing *a* is good iff doing *a* is not good, so ~Å can be read as either of the epistemically equivalent propositions _a is not good_ and _not a is good_. 
