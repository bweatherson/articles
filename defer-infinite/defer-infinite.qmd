...
Snow @Zhang2025 recently proved a result that mostly generalises Gallow's result, though it does weaken it in one crucial respect.^[Zhang's result is reported in @PettigrewWeisberg2024. The full result Zhang proves is considerably more general than this one, but the version I'll discuss makes it easier to see the contrast between the finite and infinite case.] She shows that it is impossible for A, B and C to satisfy the following five constraints.
