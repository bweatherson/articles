...
Premise 3 says that this suffices for there to be a symmetric equilibrium of the game. Again, it's worth going over the argument a bit slowly. Start, once again, with $Cr_x$, i.e., $x$'s credences about what the other players will do. We'll argue that the situation where everyone plays the mixed strategy corresponding to these credences must be an equilibrium. (By 'mixed strategy corresponding to these credences, I mean that if $Cr_x(s_y) = p$, then player $y$ in fact plays strategy $s$ with probability $p$, and so on for all players, strategies, and probabilities.) Premise 2 says that $x$ believes that everyone else has the same credences as they do. And $x$ believes that everyone is rational, i.e., an expected utility maximiser. Now consider any strategy that $x$ gives positive probability to $y$ playing. That strategy must maximise expected utility given $y$'s credences; otherwise $x$ could rule it out by the fact that $y$ is an expected utility maximiser, and give it 0 credence. And $y$'s credences, as we've seen, must 
Assume that player $x$ believes that player $y$ will play $s$, where $s$ might be some pure or mixed strategy. 


Premise 1 says that a particular world, where the players play this game under conditions of common knowledge of rationality and shared evidence, is possible. Note that one consequence of premise 1 is that this is not a dilemma; the players can make choices that maximise expected utility. So the player can look at their own credences, and infer that these are rational credences. 
