...
which I'll write as \(R^{\rightarrow}_i\), for \(i \in \mathbb{N}\), is
...
\(R^{\rightarrow}_i\) satisfies \textbf{UpR} and \textbf{RDown}. It's
only a little harder to show that it satisfies \textbf{RRef++}. Assume
\(xR^{\rightarrow}_iy\). So \(x\) is right-handed, and \(y\) is
...
at each point one or other disjunct in (\ref{Splitting}) is true.
...
pages xxx-yyy of \emph{The Connectives}), and more generally the
relationship between \(A > (B v C)\) and
...
Humberstone (\citeproc{ref-Humberstone2011}{2011, 10xx}) calls
...
(\citeproc{ref-Humberstone2011}{2011, 10xx--10xy}). So I think we
...
I think this connection, drawn by Humberstone and then again by Bacon,
is entirely correct if, \emph{but only if}, we're working in a possible
worlds framework. If we're in a possibilities framework, it doesn't look
right. It could be that the nearest possibility in which \(A \vee B\) is
not identical to either the nearest possibility in which \(A\), or the
nearest possibility in which \(B\), but is instead a coarsening of one
