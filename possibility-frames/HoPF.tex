...
\mbox{$\langle W, \leqslant \rangle$}. (I'll call
\mbox{$\langle W, \leqslant \rangle$} a \emph{possibility frame} in most
...
\mbox{$\langle W, \leqslant, V \rangle$}, where \(V\) is a function from
...
\mbox{$\langle W, \leqslant, D, V \rangle$}, where \(D\) assigns a
non-empty domain of objects to each point, and \(V\) interprets the
non-logical vocabulary. More precisely:
...
If \mbox{$\langle o_1, \dots, o_n \rangle \in V(F^n_j, x)$} and
\(x \leqslant y\), then
\mbox{$\langle o_1, \dots, o_n \rangle \in V(F^n_j, y)$}.
\item[Refinability for predicate extensions]
If \mbox{$\langle o_1, \dots, o_n \rangle \notin V(F^n_j, x)$}, then
there exists some \(y \geqslant x\) such that for all \(z \geqslant y\),
\mbox{$\langle o_1, \dots, o_n \rangle \notin V(F^n_j, z)$}.
\end{description}
...
\mbox{$\langle \llbracket t_1 \rrbracket^{g,x}, \dots, \llbracket t_n \rrbracket^{g,x} \rangle \in V(F^n_j, x)$}.
...
simply have a frame \mbox{$\langle W, \leqslant \rangle$} and a
stipulated assignment of truths at each point in \(W\). These
assignments will be presumed to satisfy persistence and refinability.
Now say that \(xR_iy\) iff \(\mathcal{M} \models_y A\) whenever
...
\mbox{$\{\mathrm{L}(\mathsf{X}) : \mathsf{X} \subseteq \mathsf{F}\}$}.
That is, \(\mathrm{ML}(\mathsf{F})\) is the class of logics that can be
...
\mbox{$\mathrm{ML}(\mathsf{K}) \subseteq \mathrm{ML}(\mathsf{H}) \subseteq \mathrm{ML}(\mathsf{FP})$}.
The fact that
...
\mbox{$s_y^R \oplus \langle 0 \rangle^R$} and
\mbox{$s_y^R \oplus \langle 1 \rangle^R$} be the right-handed sequences
...
at both \mbox{$s_y^R \oplus \langle 0 \rangle^R$} and
\mbox{$s_y^R \oplus \langle 1 \rangle^R$}. Since
\mbox{$s_y^R \oplus \langle 0 \rangle^R$} has a 0 at its \(i\)th
position, it is a dead-end with respect to \(R^{\leftarrow}_i\). So
\(\neg \Diamond_i^{\leftarrow} \mathsf{T}\) is true there. So since
\(x\) is left-handed, and \mbox{$s_y^R\oplus \langle 0 \rangle^R$} is
right-handed,
\(\Diamond^{\rightarrow} p \wedge \neg \Diamond_i^{\leftarrow} \mathsf{T}\)
is true at \(x\). A similar argument shows that
\(p \wedge \Diamond_i^{\leftarrow} \mathsf{T}\) is true at
\mbox{$s_y^R\oplus \langle 1 \rangle^R$}, so
...
\mbox{$\langle \textit{Romeo loves Juliet}\rangle$} is just Romeo and
Juliet, and universal quantifiers only range over the domain of that
...
\mbox{$\langle \textit{Romeo loves Juliet}\rangle$} would not comply
with refinability. For any object (other than Romeo and Juliet), we want
them to not be \(E\). But there is no later world where there is some
name \(c\) for them, and \(\neg Ec\) is true. So by refinability, all
objects are \(E\), defeating the point of the proposal.
...
\mbox{$\langle W, \leqslant, V \rangle$} to a conditional possibility
model by adding a selection function \(f\). This is a function
