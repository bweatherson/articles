...
To start with, assume we're working in a simple language that has a countable set $\mathcal{P}$ of propositional variables, and three connectives: $\neg$, $\wedge$, and $\vee$. We have a set of possibilities $W$, and a reflexive and transitive refinement relation $\leqslant$ on them. The following rules show how to build what I'll call a _Humberstone possibility model_ on `\mbox{$\langle W, \leqslant \rangle$}`{=latex}. (I'll call `\mbox{$\langle W, \leqslant \rangle$}`{=latex} a _possibility frame_ in most contexts, but a _Humberstone frame_ when I'm comparing it to similar structures, especially in the context of discussing @Holliday2025.)

A Humberstone possibility model $\mathcal{M}$ is a triple `\mbox{$\langle W, \leqslant, V \rangle$}`{=latex}, where $V$ is a function from $\mathcal{P}$ to $\wp(W)$, intuitively saying where each atomic proposition is true, satisfying these two constraints:
...
The second extension will be to add quantifiers, following a suggestion in @Humberstone1981a [331]. Assume, as usual, that the language has a stock of names $c_1, \dots$, and for each $n$, a stock of $n$-place predicates $F^n_1, F^n_2, \dots$. A _first-order (Humberstone) possibility model_ is a structure `\mbox{$\langle W, \leqslant, D, V \rangle$}`{=latex}, where $D$ assigns a non-empty domain of objects to each point, and $V$ interprets the non-logical vocabulary. More precisely:
...
:    If `\mbox{$\langle o_1, \dots, o_n \rangle \in V(F^n_j, x)$}`{=latex} and $x \leqslant y$, then `\mbox{$\langle o_1, \dots, o_n \rangle \in V(F^n_j, y)$}`{=latex}.

Refinability for predicate extensions
:    If `\mbox{$\langle o_1, \dots, o_n \rangle \notin V(F^n_j, x)$}`{=latex}, then there exists some $y \geqslant x$ such that for all $z \geqslant y$, `\mbox{$\langle o_1, \dots, o_n \rangle \notin V(F^n_j, z)$}`{=latex}.
...
In the special case where every name already has a denotation at $x$, the atomic clauses simplify in just the way you would hope. If $t_1, \dots, t_n$ are variables, or names that already have denotations at $x$, then by persistence of names and predicate extensions the $\forall\exists$ quantifier prefix collapses: $\mathcal{M}, g \models_x t_1 = t_2$ iff $\llbracket t_1 \rrbracket^{g,x} = \llbracket t_2 \rrbracket^{g,x}$, and $\mathcal{M}, g \models_x F^n_j(t_1, \dots, t_n)$ iff `\mbox{$\langle \llbracket t_1 \rrbracket^{g,x}, \dots, \llbracket t_n \rrbracket^{g,x} \rangle \in V(F^n_j, x)$}`{=latex}. The more complex clauses above are needed only to handle the case where some name occurring in the formula lacks a denotation at $x$ but is guaranteed to acquire one.
...
Now consider the case where $x$ is left-handed, and there are right-handed points such that $p$ is true at $y$. Here it will be helpful to denote a right-handed point $y$ as $s_y^R$. Among those $s_y^R$ at which $p$ is true, consider the ones where $|s_y|$, the length of $s_y$, is minimal. There must be some such points, since sequence lengths are non-negative integers and $p$ is true at some accessible right-handed point. Let $i = |s_y| + 1$. Let `\mbox{$s_y^R \oplus \langle 0 \rangle^R$}`{=latex} and `\mbox{$s_y^R \oplus \langle 1 \rangle^R$}`{=latex} be the right-handed sequences generated by adding either a 0 or a 1, respectively, to $s_y^R$. Since $p$ is true at $s_y^R$ and truth is persistent, $p$ will be true at both `\mbox{$s_y^R \oplus \langle 0 \rangle^R$}`{=latex} and `\mbox{$s_y^R \oplus \langle 1 \rangle^R$}`{=latex}. Since `\mbox{$s_y^R \oplus \langle 0 \rangle^R$}`{=latex} has a 0 at its $i$th position, it is a dead-end with respect to $R^{\leftarrow}_i$. So $\neg \Diamond_i^{\leftarrow} \mathsf{T}$ is true there. So since $x$ is left-handed, and `\mbox{$s_y^R\oplus \langle 0 \rangle^R$}`{=latex} is right-handed, $\Diamond^{\rightarrow} p \wedge \neg \Diamond_i^{\leftarrow} \mathsf{T}$ is true at $x$. A similar argument shows that $p \wedge \Diamond_i^{\leftarrow} \mathsf{T}$ is true at `\mbox{$s_y^R\oplus \langle 1 \rangle^R$}`{=latex}, so $\Diamond^{\rightarrow}(p \wedge \Diamond_i^{\leftarrow} \mathsf{T})$ is true at $x$. And that implies that the $i$th disjunct of the right-hand disjunction of (\ref{Splitting}) is true at $x$, as required.
...
That won't work though. If the domain of `\mbox{$\langle \textit{Romeo loves Juliet}\rangle$}`{=latex} is just Romeo and Juliet, and universal quantifiers only range over the domain of that possibility, then the sentence $\forall x(x = \mathit{Romeo} \vee x = \mathit{Juliet})$ would come out true. But by $\forall\text{-Elimitation}, we would then get that Mercutio is identical to Romeo or Mercutio is identical to Juliet. We don’t want that, because there are clearly refinements of our simple story where Mercutio is neither Romeo nor Juliet. Shakespeare’s play is one such refinement.
